...
function setupEvents() {
    ...
    searchInput.addEventListener('input', () => {
        clearTimeout(searchTimer);
        searchTimer = setTimeout(triggerSearch, 800); // Debounce de 0.8s para economizar API
    });
...
async function fetchClipMovies(query, year = '') {
    ...
    // Timer para avisar sobre lentidão
    ...
    const retryTimer = setTimeout(() => {
        ...
        if (wrap) {
            wrap.innerHTML = `
                <p style="color:#ffcc00">Servidor demorando para entregar filmes. Quer tentar de novo?</p>
    ...
    try {
        ...
        // Timeout estendido de monitoramento (35s)
        ...
        if (!res.ok) {
            if (res.status === 429) throw new Error("Limite de busca atingido (API). Aguarde 1 minuto.");
            throw new Error(`Servidor da Clip.Cafe lento ou fora do ar (${res.status})`);
        }
        const data = await res.json();
        console.timeEnd('BuscaFilmes');
...
async function openClipMovie(movie) {
    ...
    // Fetch ALL scenes (limit 300) with browser headers to avoid 403
    try {
...
async function downloadFileNode(url, dest, _attempt) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Remove arquivo apenas na primeira tentativa de um download novo
        ...
        const req = protocol.get(url, options, (res) => {
            ...
            if ([301, 302, 307, 308].includes(res.statusCode)) {
                res.resume(); // Consome para liberar socket
            ...
            const watchdog = setInterval(() => {
                if (Date.now() - lastDataTime > 20000) { // 20s sem novos dados = download travado
                    isDone = true;
                    clearInterval(watchdog);
                    req.destroy();
                    fileStream.destroy();
                    if (attempt < MAX_ATTEMPTS) {
                        console.log('[Download] Inativo. Reiniciando tentativa...');
                        downloadFileNode(url, dest, attempt + 1).then(resolve).catch(reject);
                    } else {
                        reject(new Error('Download cancelado por inatividade do servidor'));
                    }
                }
            }, 5000);
